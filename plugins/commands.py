...
async def start(client, message):
    ...
    if not files_:
        ...
        try:
            ...
            k = await msg.reply("<b><u>IMPORTANT</u></b>\n\nThis Movie File/Video will be deleted in <b>10 mins<i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
...
async def log_file(bot, message):
    try:
        await message.reply_document('MovieVerse Logs.LOG')
    except Exception as e:
    ...
    if message.reply_to_message and SUPPORT_CHAT_ID == message.chat.id:
        ...
        try:
            if REQST_CHANNEL is not None:
                ...
                reported_post = await bot.send_message(chat_id=REQST_CHANNEL, text=f"<b>Reporter: {mention} ({reporter})Message {content}</b>", reply_markup=InlineKeyboardMarkup(btn))
                success = True
            elif len(content) >= 3:
                for admin in ADMINS:
                    btn = [[
                    ...
                    reported_post = await bot.send_message(chat_id=admin, text=f"<b>Reporter: {mention} ({reporter})Message {content}</b>", reply_markup=InlineKeyboardMarkup(btn))
                    success = True
            else:
                if len(content) < 3:
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        btn = [ 
            [InlineKeyboardButton("ɢᴇᴛ ғʀᴇᴇ ᴛʀᴀɪʟ ғᴏʀ 𝟻 ᴍɪɴᴜᴛᴇꜱ", callback_data="get_trail")],
